...
export function AIFlexibility({
  ...
  return <div className={`w-full ${isDark ? 'bg-slate-950' : 'bg-white'} py-24 px-6`}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className={`p-8 md:p-12 rounded-2xl ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-gray-50 border-gray-200'} border mb-12`}>
          <h3 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} mb-8 text-center`}>
            Supported AI Providers
          </h3>
          <div className="grid md:grid-cols-3 gap-6">
            {aiProviders.map((provider, index) => <div key={index} className={`p-6 rounded-xl ${isDark ? 'bg-slate-900/50 border-slate-700' : 'bg-white border-gray-200'} border`}>
                <div className="flex items-start justify-between mb-3">
                  <h4 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    {provider.name}
                  </h4>
                  <CheckCircleIcon className={`w-5 h-5 ${isDark ? 'text-green-400' : 'text-green-600'} flex-shrink-0`} />
                </div>
        ...
        <div className={`p-8 rounded-2xl ${isDark ? 'bg-gradient-to-r from-blue-900/20 to-purple-900/20 border-blue-500/30' : 'bg-gradient-to-r from-red-50 to-orange-50 border-red-200'} border`}>
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              ...
              <ul className="space-y-3">
                <li className="flex items-start gap-3">
                  <CheckCircleIcon className={`w-5 h-5 ${isDark ? 'text-blue-400' : 'text-red-600'} flex-shrink-0 mt-1`} />
            ...
            <div className={`p-6 rounded-xl ${isDark ? 'bg-slate-800/50' : 'bg-white'} text-center`}>
              <div className={`text-5xl font-bold ${isDark ? 'text-blue-400' : 'text-red-600'} mb-2`}>
                6+
              </div>
              <p className={`text-lg ${isDark ? 'text-slate-300' : 'text-gray-700'} font-semibold mb-2`}>
