...
import { DatabaseIcon, ArrowRightIcon, BrainIcon, TrendingUpIcon } from 'lucide-react';
...
export function ProblemSolution({
  ...
  const crmPartners = ['Salesforce', 'HubSpot', 'Encompass', 'Blend', 'Calyx Point', 'BytePro', 'Total Expert', 'Velocify', 'Surefire', 'LoanLogics'];
  return <div className={`w-full ${isDark ? 'bg-gradient-to-b from-slate-950 to-slate-900' : 'bg-gradient-to-b from-white to-gray-50'} py-24 px-6`}>
      <div className="max-w-6xl mx-auto">
        ...
        {/* Step 1: The Problem */}
        <div className="mb-16">
          <div className={`p-8 rounded-2xl ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-gray-200'} border backdrop-blur-sm`}>
            <div className="flex items-start gap-6">
              <div className={`w-16 h-16 rounded-xl ${isDark ? 'bg-red-500/10' : 'bg-red-100'} flex items-center justify-center flex-shrink-0`}>
                <DatabaseIcon className={`w-8 h-8 ${isDark ? 'text-red-400' : 'text-red-600'}`} />
              </div>
              <div>
                <h3 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} mb-3`}>
                  1. The Problem: Valuable Data Sits Disconnected
                </h3>
                <p className={`${isDark ? 'text-slate-300' : 'text-gray-600'} text-lg leading-relaxed mb-4`}>
        ...
        <div className="flex justify-center mb-16">
          <ArrowRightIcon className={`w-12 h-12 ${isDark ? 'text-blue-400' : 'text-red-600'} transform rotate-90`} />
        </div>
        {/* Step 2: Connect Your Stack */}
        <div className="mb-16">
          <div className={`p-8 rounded-2xl ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-gray-200'} border backdrop-blur-sm`}>
            <div className="flex items-start gap-6">
              <div className={`w-16 h-16 rounded-xl ${isDark ? 'bg-blue-500/10' : 'bg-blue-100'} flex items-center justify-center flex-shrink-0`}>
                <BrainIcon className={`w-8 h-8 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} />
              </div>
              <div className="flex-1">
                <h3 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} mb-3`}>
                  2. Connect to Your Existing Tech Stack
                </h3>
                <p className={`${isDark ? 'text-slate-300' : 'text-gray-600'} text-lg leading-relaxed mb-6`}>
                ...
                <div className={`p-6 rounded-xl ${isDark ? 'bg-slate-900/50' : 'bg-gray-50'}`}>
                  <p className={`${isDark ? 'text-slate-400' : 'text-gray-600'} font-semibold mb-4`}>
                    API-Enabled CRM Integrations:
                  </p>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    {crmPartners.map((crm, index) => <div key={index} className={`px-4 py-2 rounded-lg ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'} border text-center`}>
        ...
        {/* Step 3: The Solution */}
        <div>
          <div className={`p-8 rounded-2xl ${isDark ? 'bg-gradient-to-br from-blue-900/20 to-purple-900/20 border-blue-500/30' : 'bg-gradient-to-br from-red-50 to-orange-50 border-red-200'} border backdrop-blur-sm`}>
            <div className="flex items-start gap-6">
              <div className={`w-16 h-16 rounded-xl ${isDark ? 'bg-green-500/10' : 'bg-green-100'} flex items-center justify-center flex-shrink-0`}>
                <TrendingUpIcon className={`w-8 h-8 ${isDark ? 'text-green-400' : 'text-green-600'}`} />
              </div>
              <div>
                <h3 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} mb-3`}>
                ...
                <div className="grid md:grid-cols-2 gap-4 mt-6">
                  <div className={`p-4 rounded-lg ${isDark ? 'bg-slate-800/50' : 'bg-white'}`}>
                    <p className={`font-semibold ${isDark ? 'text-blue-300' : 'text-red-600'} mb-2`}>
                      Backend Analytics
                    </p>
                  ...
                  <div className={`p-4 rounded-lg ${isDark ? 'bg-slate-800/50' : 'bg-white'}`}>
                    <p className={`font-semibold ${isDark ? 'text-purple-300' : 'text-orange-600'} mb-2`}>
                      Consumer-Facing AI
                    </p>
                    <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>
